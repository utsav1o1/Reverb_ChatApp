            ...
            @foreach ($messages as $message)
                @if ($message['sender'] != auth()->user()->name)
                    <div class="clearfix w-4/4">
                        <div class="inline-block p-2 mx-4 my-2 bg-gray-300 rounded-lg "><b>{{$message['sender']}}: </b>{{ $message['message'] }}</div>
                    </div>
                @else
                <div class="clearfix w-4/4">
                    <div class="text-right">
                        <p class="inline-block p-2 mx-4 my-2 bg-green-300 rounded-lg ">{{$message['message']}} <b>:You</b></p>
                    </div>
                </div>
                @endif
            @endforeach
    ...
    <form wire:submit="sendMessage">
        <div class="fixed flex justify-between w-full bg-green-100" style="bottom: 0px;">
            <textarea wire:model="message"
                class="flex-grow px-4 py-2 m-2 mr-1 bg-gray-200 border border-gray-300 rounded-full resize-none" rows="1"
